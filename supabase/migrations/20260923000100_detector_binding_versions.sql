-- Named class expressions and destructuring writes have corrected binding semantics.
-- Append detector 1.0.3; structural extractors and coverage calculations are unchanged.
-- All previously sealed reports and detector definitions retain their original identities.
INSERT INTO feature_one_private.implementation_detectors(bundle_version,kind,capabilities,maximum_strength)
  SELECT '1.0.3',kind,capabilities,maximum_strength FROM feature_one_private.implementation_detectors WHERE bundle_version='1.0.2';
...
BEGIN
  ...
  IF base_version NOT IN ('1.0.0','1.0.1','1.0.2','1.0.3') THEN RAISE EXCEPTION 'INCOMPLETE_ANALYSIS'; END IF;
