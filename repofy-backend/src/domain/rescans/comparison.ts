...
export function compareReports(input: ComparisonInput, query: ComparisonQuery): Comparison {
  ...
  // Equal limit counters do not prove that the same files were selected: lexical
  // ordering can displace unchanged source on either side of the detector budget.
  if ([...a.report.coverage, ...b.report.coverage].some(incomplete)) causes.add("scope_incomplete");
