...
function legacy(bundle: any, declaration: unknown, version: '1.0.0' | '1.0.1' | '1.0.2') {
...
export function registerAnalyzerVersionTests(db: pg.Client) {
  test('analyzer corrections append immutable definitions and preserve every legacy coverage declaration', async () => {
    ...
    assert.deepEqual(detectors.rows, [{ bundle_version: '1.0.0', n: 16 }, { bundle_version: '1.0.1', n: 16 }, { bundle_version: '1.0.2', n: 16 }, { bundle_version: '1.0.3', n: 16 }]);
  ...
  for (const version of ['1.0.0', '1.0.1', '1.0.2'] as const) test(`${version} snapshot and evidence remain readable while rejecting mismatched rows`, async () => {
    const fixture = await coverageJobFixture(db, jobsRpc(db), files);
    try {
      const args = await captured(fixture);
      ...
      for (const kind of ['implementation', 'schema']) {
        ...
        observation.detector.version = kind === 'implementation' ? '1.0.3' : version === '1.0.0' ? '1.0.1' : '1.0.0';
