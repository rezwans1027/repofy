...
describe("TopBanner", () => {
  ...
  it("applies emerald color at exact boundary of 80", () => {
    const boundaryData = createReportFixture({ overallScore: 80 });
    const { container } = render(<TopBanner username="testuser" data={boundaryData} />);
    const scoreEl = container.querySelector(".text-emerald-400");
    expect(scoreEl).toBeInTheDocument();
  });

  it("applies yellow color at exact boundary of 60", () => {
    const boundaryData = createReportFixture({ overallScore: 60 });
    const { container } = render(<TopBanner username="testuser" data={boundaryData} />);
    const scoreEl = container.querySelector(".text-yellow-400");
    expect(scoreEl).toBeInTheDocument();
  });

  it("SVG circle has correct stroke-dasharray circumference", () => {
    const { container } = render(<TopBanner username="testuser" data={data} />);
    const circles = container.querySelectorAll("circle");
    const circumference = `${2 * Math.PI * 42}`;
    const hasCircumference = Array.from(circles).some(
      (circle) => circle.getAttribute("stroke-dasharray")?.includes(circumference.slice(0, 6)),
    );
    expect(hasCircumference).toBe(true);
  });
});
