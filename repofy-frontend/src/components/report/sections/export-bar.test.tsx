...
describe("ExportBar", () => {
  ...
  it("does not call exportToPdf when ref.current is null", async () => {
    const { exportToPdf } = await import("@/lib/export-pdf");
    ...
    render(
      <ExportBar username="testuser" reportRef={nullRef} onBeforeExport={onBefore} onAfterExport={onAfter} />
    );
    fireEvent.click(screen.getByText("Export PDF"));
  ...
  it("shows Exporting... text during export", async () => {
    ...
    render(
      <ExportBar username="testuser" reportRef={divRef} onBeforeExport={vi.fn()} onAfterExport={vi.fn()} />
    );
    fireEvent.click(screen.getByText("Export PDF"));
  ...
  it("calls onAfterExport after export completes", async () => {
    const div = document.createElement("div");
    const divRef = { current: div };
    const onAfter = vi.fn();

    render(
      <ExportBar username="testuser" reportRef={divRef} onBeforeExport={vi.fn()} onAfterExport={onAfter} />
    );
    fireEvent.click(screen.getByText("Export PDF"));
