...
test.describe.skip("Reports page", () => {
  ...
  test("reports page sort dropdown works", async ({ page }) => {
    // Seed a second report so sort order is verifiable
    await seedReportViaApi(page, "torvalds");
    ...
    // Select "Highest score" and verify sort order
    ...
    const scoreTexts = await page
      ...
      .allTextContents();
    const scores = scoreTexts.map(Number);
  ...
  test("clicking a report username navigates to report detail", async ({
    ...
    const reportLink = page.getByTestId("report-table-link").first();
  ...
  test.afterEach(async ({ page }) => {
    await deleteSeededReports(page, ["octocat", "torvalds"]);
  });
});
