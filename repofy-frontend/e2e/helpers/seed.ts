...
function getSupabaseConfig(): { url: string; anonKey: string } {
  ...
  // Fall back to reading .env.local for local development
...
export async function seedReportViaApi(
  ...
  const { analyzedName, report } = await generateReportViaApi(page, username);
  ...
  const upsertResp = await page.request.post(
    `${supabaseUrl}/rest/v1/reports?on_conflict=user_id,analyzed_username`,
    {
      headers: {
        apikey: anonKey,
...
export async function deleteSeededReports(
  ...
  const resp = await page.request.delete(
    `${supabaseUrl}/rest/v1/reports?user_id=eq.${userId}&analyzed_username=in.(${filter})`,
    {
      headers: {
        apikey: anonKey,
        Authorization: `Bearer ${accessToken}`,
  ...
  if (!resp.ok()) {
    const msg = `cleanup: delete reports returned ${resp.status()}`;
    if (process.env.CI) {
      throw new Error(msg);
    }
    console.warn(msg);
