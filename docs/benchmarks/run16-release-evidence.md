...
The first gate run exposed a measurement defect: fresh test repository UUIDs can change which equally ranked observation supplies a capability's narrative (C02). The benchmark binds labels to a frozen actual rendering matching the approved text and validates its corpus and rendering hashes. Fresh-run differences are listed separately in `reviewedRendering.differingReviewCards`; they do not inherit approval. Six diagnostic replays exercised both C02 selections while retaining the same reviewed sample. This fix changes the benchmark, not production tie-breaking or approved wording. Historical review metrics explicitly describe the frozen sample.

The subsequent branch review fixes change the domain implementation digest, so that frozen approval is **stale for the current implementation**. Automated benchmarks continue against the unchanged corpus, while `humanReview.status` records `stale`, current human support/error rates remain `null`, and the original judgments appear only under `historicalReview`. The original human review and frozen rendering are retained unchanged. A current implementation with no judgments instead records `unreviewed`; neither state passes `human_boundary_sample`, and the rollout decision remains HOLD. A new human review must be explicitly bound to its own frozen rendering and domain digest before current human metrics can be measured. Missing or corrupted corpus, rendering, rubric or review bindings still fail validation.
...
| ANA-007 supported major claims | [narrative validator](../../repofy-backend/src/domain/synthesis/narrative.ts) | B forbidden claims/IDs/disclosure; PG tampering; 58/58 major references; 22 historically reviewed claims | Small sample; new bound current review, second independent review and pinned-model success — evaluation/model owners |
