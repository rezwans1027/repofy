...
The crypto class accepts a versioned injected keyring. New writes use the active key; retain old keys while immutable artifacts or recoverable backups need them. Changing an environment key under an existing version breaks decryption. This run does not install KMS infrastructure or a ciphertext rewrite job. Rotation uses a new version plus an injected ring containing retained old keys; irreversible key removal must follow the deployment's documented backup/retention process.
